...
void ZRenderArea::doneConnBuilder(bool none, int type, int pinNum, ZCPInput* input, ZCPOutput* output, ZCPBase *finishFilter)
{
    ...
    CStructures::PinClass inputClass = CStructures::PinClass::pcPCM;
    CStructures::PinClass outputClass = CStructures::PinClass::pcPCM;
    if (m_startPinType==ZCPBase::PinType::ptInput) {
...
bool ZRenderArea::postLoadBinding() const
{
    const auto cplist = findComponents<ZCPBase*>();
    for (const auto& cp : cplist) {
        if (!(cp->postLoadBind()))
...
void ZRenderArea::doGenerate(QTextStream & stream, QStringList & warnings) const
{
    stream << "# This file is automatically generated in ALSA Plugin Editor v2." << Qt::endl
           << "# " << QDateTime::currentDateTime().toString(Qt::ISODate) << Qt::endl << Qt::endl;
